...
module YEA
  module SAVE
    ...
    DELETE_HELP = "WARNING! Deletes all data from this save file."
    ...
    PLAYTIME   = "Time"              # Text used for total playtime.
    TOTAL_SAVE = ""                  # Text used to indicate total saves.
...
class Window_FileStatus < Window_Base
  ...
  def draw_save_gold(dx, dy, dw)
    ...
    draw_icon(Icon.gold, dx+dw-24, dy)
    #cx = text_size(text).width
    change_color(normal_color)
    text = @header[:party].gold.group
    #draw_text(dx, dy, dw-cx, line_height, text, 2)
    draw_text(dx, dy, dw-26, line_height, text, 2)
  end
  ...
  def draw_save_characters(dx, dy)
    ...
    for member in @header[:party].battle_members
      ...
      dx += 32
      @header[:party].all_members.each do |actor|
        draw_actor_face(actor, dx-32, dy-48)
        dx += 56
      end
      #text = member.name
  ...
  def draw_column_data(data, dx, dy, dw)
    ...
    for variable_id in data
      ...
      draw_text(dx + 32, dy, dw, line_height, value, 2)
      dy += line_height
    end
  end
  ...
  def draw_save_contents
    ...
    draw_save_column1(16, line_height*6, contents.width/2-48)
    change_color(system_color)
    draw_text(16, line_height*10, contents.width, line_height, "Name", 0)
    change_color(normal_color)
    draw_text(75, line_height*10, contents.width, line_height, @header[:actors][8].name, 0)
    draw_save_column2(contents.width/2+16, line_height*6, contents.width/2-48)
  end
