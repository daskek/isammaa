...
module YEA
  module PARTY
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    MAX_BATTLE_MEMBERS   = 1      # Maximum party members. Default: 4
    SPLIT_EXP            = true   # Splits EXP with more members in the party.
    ...
    PARTY_FONT_SIZE  = 24    # Font size used for party member names.
    ...
    EMPTY_TEXT       = "No Pokémon"   # Text used when a member isn't present.
    ...
    NO_DATA         = "No Pokémon" # Text used for when no actor is shown.
    IN_PARTY_COLOUR = 6             # Text colour used for in party members.
    STAT_FONT_SIZE  = 24            # Font size used for stats.
    EQUIP_TEXT      = ""   # Text used to display equipment.
...
class Window_PartyStatus < Window_Base
  ...
  def refresh
    ...
    draw_actor_exp(actor, 108, line_height*3, contents.width-112)
...
class Scene_Party < Scene_MenuBase
  ...
  def on_list_ok
    ...
    window_refresh
    @party_window.activate
    $game_variables[17] = $game_party.all_members.first.class.id
    if $game_variables[16] != $game_variables[17]
