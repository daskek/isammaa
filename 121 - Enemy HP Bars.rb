...
class Sprite_Battler
  ...
  def update_hp_bar
    ...
    if DTP_HP::USE_TEXT and !@battler.boss?
      if @battler.hide_name
        ...
        if $game_variables[38] > 0
          if $game_variables[38] > 99
            @hp_bar.draw_text(0,yy+36,width-32,24,"Lv.",2)
          elsif $game_variables[38] > 9
            @hp_bar.draw_text(0,yy+36,width-24,24,"Lv.",2)
          else
            @hp_bar.draw_text(0,yy+36,width-16,24,"Lv.",2)
          end
          @hp_bar.draw_text(0,yy+36,width,24,$game_variables[38].to_s,2)
        end
      end
      yy += 24
    end
    ...
    if DTP_HP::USE_STATES
      ...
      @battler.states.each do |state|
        @hp_bar.draw_icon(state.icon_index,xx,yy+12)
        xx += 24
      end
    end
  end
  def setup_bar
    ...
    if @battler.boss?
      ...
      @hp_bar = Window_Base.new(0,0,180,height)
