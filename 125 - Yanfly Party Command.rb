...
module YEA
  module COMMAND_PARTY
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    COMMAND_TEXT   = "Pokémon"  # Text used for the command.
    PARTY_COOLDOWN = 0          # Turns that must pass between each change.
    SHOW_SWITCH    = 11         # If switch is on, show command. 0 to disable.
    ENABLE_SWITCH  = 12         # If switch is on, enable command. 0 to disable.
...
class Game_Unit
  ...
  def battle_party_change?
    ...
    return false if $game_switches[15]
    reset_party_cooldown if @party_cooldown.nil?
...
class Scene_Battle < Scene_Base
  ...
  #--------------------------------------------------------------------------
  def command_party
    $game_variables[16] = $game_party.all_members.first.id
    Graphics.freeze
