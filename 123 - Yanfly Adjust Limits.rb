...
ITEM_FONT = 24 # Font size used to display item quantity.
SHOP_FONT = 24 # Font size used for shop item costs.
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LEVEL_MAX = 100 # Sets max level to x for those with 99 level limit.
MAXHP_MAX = 99999 # Sets MaxHP to something higher than 9999.
MAXMP_MAX = 99999 # Sets MaxMP to something higher than 9999.
PARAM_MAX = 9999 # Sets stat max for something higher than 999.
EQUIP_FONT = 24 # Changes the default equip window font size.
end # LIMIT
end # YEA
#==============================================================================
...
if actor.level.to_i == 100
  draw_text(dx + cx - 24, dy, dw, line_height, actor.level.group, 2)
elsif actor.level.to_i > 9
  draw_text(dx + cx - 34, dy, dw, line_height, actor.level.group, 2)
else
  draw_text(dx + cx - 44, dy, dw, line_height, actor.level.group, 2)
end
end
#--------------------------------------------------------------------------
